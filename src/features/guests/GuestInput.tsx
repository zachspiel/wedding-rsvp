    ...
    <>
      ...
      <Grid>
        ...
        <Grid.Col span={2}>
          {index !== 0 && (
            <MGroup>
              <ActionIcon onClick={() => form.removeListItem("guests", index)}>
                <IconX size="1.125rem" />
              </ActionIcon>
            </MGroup>
          )}
      ...
      {guest.relationshipType === PRIMARY && showAddPlusOneButton && (
        <Button
          variant="outline"
          onClick={() => addPartnerToGuests(form)}
          mt="lg"
          mr="md"
        >
          Add Plus One
      ...
      {index > 0 && guest.relationshipType !== PRIMARY && (
        <Switch
          mt="lg"
          label="Name Unknown"
          onChange={(event) => setNameUnknown(event.currentTarget.checked)}
        />
      )}
      {index === guests.length - 1 && groupType === "family" && (
        <Button
          variant="outline"
          onClick={() => addChildToGuests(form)}
          mt="lg"
        >
          Add Child
        </Button>
